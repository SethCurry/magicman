...
type CardQuery struct {
	...
	withSet        *SetQuery
	withTypes      *CardTypeQuery
	withSubtypes   *SubTypeQuery
	withSupertypes *SuperTypeQuery
	withColors     *ColorQuery
	withDeckCards  *DeckCardQuery
	withFKs        bool
...
func (cq *CardQuery) QueryRulings() *RulingQuery {
	query := &RulingQuery{config: cq.config}
...
func (cq *CardQuery) QuerySet() *SetQuery {
	query := &SetQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CardQuery) QueryTypes() *CardTypeQuery {
	query := &CardTypeQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySubtypes chains the current query on the "subtypes" edge.
func (cq *CardQuery) QuerySubtypes() *SubTypeQuery {
	query := &SubTypeQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySupertypes chains the current query on the "supertypes" edge.
func (cq *CardQuery) QuerySupertypes() *SuperTypeQuery {
	query := &SuperTypeQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryColors chains the current query on the "colors" edge.
func (cq *CardQuery) QueryColors() *ColorQuery {
	query := &ColorQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryDeckCards chains the current query on the "deck_cards" edge.
func (cq *CardQuery) QueryDeckCards() *DeckCardQuery {
	query := &DeckCardQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CardQuery) Clone() *CardQuery {
	...
	return &CardQuery{
		...
		withSet:        cq.withSet.Clone(),
		withTypes:      cq.withTypes.Clone(),
		withSubtypes:   cq.withSubtypes.Clone(),
		withSupertypes: cq.withSupertypes.Clone(),
		withColors:     cq.withColors.Clone(),
		withDeckCards:  cq.withDeckCards.Clone(),
...
func (cq *CardQuery) WithSet(opts ...func(*SetQuery)) *CardQuery {
	query := &SetQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CardQuery) WithTypes(opts ...func(*CardTypeQuery)) *CardQuery {
	query := &CardTypeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// the "subtypes" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CardQuery) WithSubtypes(opts ...func(*SubTypeQuery)) *CardQuery {
	query := &SubTypeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withSubtypes = query
	return cq
}

// WithSupertypes tells the query-builder to eager-load the nodes that are connected to
// the "supertypes" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CardQuery) WithSupertypes(opts ...func(*SuperTypeQuery)) *CardQuery {
	query := &SuperTypeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withSupertypes = query
	return cq
}

// WithColors tells the query-builder to eager-load the nodes that are connected to
// the "colors" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CardQuery) WithColors(opts ...func(*ColorQuery)) *CardQuery {
	query := &ColorQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// WithDeckCards tells the query-builder to eager-load the nodes that are connected to
// the "deck_cards" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CardQuery) WithDeckCards(opts ...func(*DeckCardQuery)) *CardQuery {
	query := &DeckCardQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withDeckCards = query
	return cq
}
...
//
//	client.Card.Query().
//		GroupBy(card.FieldMultiverseID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (cq *CardQuery) sqlAll(ctx context.Context) ([]*Card, error) {
	var (
		...
		loadedTypes = [7]bool{
			...
			cq.withTypes != nil,
			cq.withSubtypes != nil,
			cq.withSupertypes != nil,
			cq.withColors != nil,
			cq.withDeckCards != nil,
		}
	)
	if cq.withSet != nil {
		withFKs = true
	...
	if query := cq.withRulings; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Rulings = []*Ruling{}
		}
		query.withFKs = true
		query.Where(predicate.Ruling(func(s *sql.Selector) {
			s.Where(sql.InValues(card.RulingsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.card_rulings
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "card_rulings" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "card_rulings" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Rulings = append(node.Edges.Rulings, n)
		}
	}

	if query := cq.withSet; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Card)
		for i := range nodes {
			if nodes[i].set_cards == nil {
				continue
			}
			fk := *nodes[i].set_cards
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "set_cards" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Set = n
			}
	...
	if query := cq.withTypes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Card, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Types = []*CardType{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(card.TypesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "types": %w`, err)
		}
		query.Where(cardtype.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "types" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Types = append(nodes[i].Edges.Types, n)
			}
		}
	}

	if query := cq.withSubtypes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Card, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Subtypes = []*SubType{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Card)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(card.SubtypesPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "subtypes": %w`, err)
		}
		query.Where(subtype.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "subtypes" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Subtypes = append(nodes[i].Edges.Subtypes, n)
			}
		}
	}

	if query := cq.withSupertypes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Card, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Supertypes = []*SuperType{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Card)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(card.SupertypesPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "supertypes": %w`, err)
		}
		query.Where(supertype.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "supertypes" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Supertypes = append(nodes[i].Edges.Supertypes, n)
			}
		}
	}

	if query := cq.withColors; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Card, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Colors = []*Color{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Card)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(card.ColorsPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "colors": %w`, err)
		}
		query.Where(color.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "colors" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Colors = append(nodes[i].Edges.Colors, n)
			}
		}
	}

	if query := cq.withDeckCards; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Card)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.DeckCards = []*DeckCard{}
		}
		query.withFKs = true
		query.Where(predicate.DeckCard(func(s *sql.Selector) {
			s.Where(sql.InValues(card.DeckCardsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.card_deck_cards
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "card_deck_cards" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "card_deck_cards" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.DeckCards = append(node.Edges.DeckCards, n)
		}
	}
