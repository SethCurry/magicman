...
func (tx *Tx) init() {
	...
	tx.CardType = NewCardTypeClient(tx.config)
	tx.Color = NewColorClient(tx.config)
	tx.Deck = NewDeckClient(tx.config)
	tx.DeckCard = NewDeckCardClient(tx.config)
	tx.Ruling = NewRulingClient(tx.config)
	tx.Set = NewSetClient(tx.config)
	tx.SubType = NewSubTypeClient(tx.config)
	tx.SuperType = NewSuperTypeClient(tx.config)
}
